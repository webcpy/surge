...
!(async () => {
  ...
  const fn = async (key, index) => {
    ...
    const KEY_FEISHU = `@ChinaTelecomOperators.${key}.feishu`
    ...
    await notify(title, subtitle, body, { copy, KEY_PUSHDEER, KEY_BARK, KEY_FEISHU })
    $.log(`👉🏻 [${index}][${key}] 配置结束`)
  }
...
async function notify(title, subtitle, body, { copy, KEY_PUSHDEER, KEY_BARK, KEY_FEISHU }) {
  const pushdeer = $.getdata(KEY_PUSHDEER)
  const feishu = $.getdata(KEY_FEISHU)
  const bark = $.getdata(KEY_BARK)

  if (pushdeer || bark || feishu) {
    if (pushdeer) {
      try {
    ...
    if (feishu) {
      try {
        ...
        console.log(e)
        $.msg('短信转发', `❌ feishu 请求`, `${$.lodash_get(e, 'message') || $.lodash_get(e, 'error') || e}`, {})
      }
    }
